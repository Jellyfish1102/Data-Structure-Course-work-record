...
class Hashing {
  private:
    ...
    void searchTable( item* table, string method, int dataSize );
...
void Hashing::quadraticProbing( vector<student> data, string fileName ) {
  ...
  searchTable( table, "Quadratic probing", data.size() );
...
void Hashing::doubleHash( vector<student> data, string fileName ) {
  ...
  searchTable( table, "Double hashing", data.size() );
...
int Hashing::getTableSize( vector<student> data ) {
  int tableSize = 1.15 * data.size() + 1;
...
// ============================ Hashing::searchTable() ============================= //
void Hashing::searchTable( item* table, string method, int dataSize ) {
  char inputID[10];
  item data;
  ...
  while ( strcmp( inputID, "0" ) ) {
    bool done = false;
    bool unsuccessful = false;
    int hvalue = hashing( inputID );
    int prob = 1;
    int location = hvalue;
    ...
    if ( method == "Quadratic probing" ) {
      ...
      while ( done == false ) {
        ...
        if ( strcmp(table[location].sid, inputID) == 0 ) {
          done = true;
          data= table[location];
        }  // if
        else if (table[location].hvalue != -1 ) {
          prob = prob + 1;
          quadraticNum = quadraticNum + 1;
          location = ( hvalue + quadraticNum * quadraticNum ) % tablesize;
        }  // else if
    ...
    // search double probing
      ...
      int increaseStep = step( dataSize, inputID );
    ...
    // if found -> show found data, prob time; not found -> show prob time, inputID
    if ( unsuccessful == false ) {
      cout << endl << "{ " << data.sid << ", " << data.sname << ", " << defaultfloat << data.mean
            << " } is found after " << prob << " probes." << endl;
    }  // if
    else {
      cout << endl << inputID << " is not found after " << prob << " probes." << endl;
    }  // else

    // input next inputID
    cout << endl << "Input a student ID to search ([0] Quit): " ;
    cin >> inputID;
  }  // while
