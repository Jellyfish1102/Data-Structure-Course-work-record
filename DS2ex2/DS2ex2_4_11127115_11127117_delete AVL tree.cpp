...
class AVLTree: public Tree {
  private:
    ...
    void deleteNode( treeNode * & inPtr, node item );
    void getSuccessor( treeNode *& node );
...
int main() {
  ...
  while ( done == false ) {
    ...
    cout << endl << "* 3. Delete item form AVL tree*";
    cout << endl << "*******************************";
...
void AVLTree::deleteAVLtree() {
  ...
  cout << endl << "input delete student number: ";
  cin >> deleteNum;
...
/*============================== AVLTree::deleteNode() ============================*/
void AVLTree::deleteNode( treeNode * & inPtr, node item ) {
  if ( inPtr != NULL ) {
    treeNode * temp = inPtr;

    if ( item.key > temp -> item.key ) {
      deleteNode( temp -> right, item );
    }  // if
    else if ( item.key < temp -> item.key ) {
      deleteNode( temp -> left, item );
    }  // else if
    else if ( item.key == temp -> item.key ) {
      if ( temp -> left != NULL && temp -> right != NULL ) {
        getSuccessor( temp );
        inPtr -> item = temp -> item;
      }  // if
  ...
  if ( inPtr != NULL ) {
    // update balance factor of current node
    inPtr -> BF = getHeight(inPtr -> left) - getHeight(inPtr -> right);

    // if right and left subtree of current node are not balance -> rotate to make it balance
    while ( inPtr -> BF == 2 || inPtr -> BF == -2 ) {
      rotateNode( inPtr );
...
/*============================= AVLTree::getSuccessor() ===========================*/
void AVLTree::getSuccessor( treeNode * & node ) {
