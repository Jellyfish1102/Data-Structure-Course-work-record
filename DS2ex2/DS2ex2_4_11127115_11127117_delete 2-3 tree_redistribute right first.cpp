...
void TwoThreeTree::fix( treeNode * & temp ) {
  ...
  if ( parent -> left == temp ) {
    cout << " left" << endl;
    borrowPtr = parent -> left;

    // parent is 3-node
    if ( parent -> item.size() == 2 ) {
      if ( parent -> middle -> item.size() == 2 )
        lendPtr = parent -> middle;

    }  // if

    // parent is 2-node
    else {
      if ( parent -> right -> item.size() == 2 )
        lendPtr = parent -> right;

    }  // else

  }  // if

  else if ( parent -> middle == temp ) {
    cout << " middle " << endl;
    borrowPtr = parent -> middle;

    if ( parent -> left -> item.size() == 2 )
      lendPtr = parent -> left;
...
void TwoThreeTree::redistribute( treeNode * & borrowPtr, treeNode * & lendPtr ) {
  ...
  if ( lendPtr -> item.front().key > parent -> item.back().key ) {
    ...
    if ( lendPtr -> left != NULL ) {
      if ( borrowPtr -> right != NULL ) {
        borrowPtr -> left = borrowPtr -> right;
      }  // if

      borrowPtr -> right = lendPtr -> left;
      lendPtr -> left -> previous = borrowPtr;
      lendPtr -> left = lendPtr -> middle;
      lendPtr -> middle = NULL;
    }  // if
  ...
  // lendPtr is parent -> left
    ...
    if ( lendPtr -> right != NULL ) {
      if ( borrowPtr -> left != NULL ) {
        borrowPtr -> right = borrowPtr -> left;
      }  // if

      borrowPtr -> left = lendPtr -> right;
      lendPtr -> right -> previous = borrowPtr;
      lendPtr -> right = lendPtr -> middle;
      lendPtr -> middle = NULL;
    }  // if
  ...
  // lendPtr is parent -> middle
    ...
    else if ( borrowPtr == parent -> right ) {
      borrowPtr ->item.push_back( parent -> item.back() );
...
void TwoThreeTree::mergeNode( treeNode * & borrowPtr ) {
  treeNode * parent = borrowPtr -> previous;
  treeNode * child = borrowPtr -> left;
  treeNode * lendPtr = NULL;

  if ( child == NULL && borrowPtr -> right != NULL ) {
    child = borrowPtr -> right;
    borrowPtr -> right = NULL;
  }  // if
  ...
  if ( borrowPtr == root ) {
    ...
    if ( borrowPtr == parent -> middle ) {
      lendPtr = parent -> right;
      if ( child != NULL ) {
        lendPtr -> middle = lendPtr -> left;
        lendPtr -> left = child;
        child -> previous = lendPtr;
      }  // if

      lendPtr -> item.insert( lendPtr -> item.begin(), parent -> item.back() );
      ...
      if (lendPtr == parent -> middle ) {
        parent -> right = lendPtr;
        parent -> middle = NULL;
      }  // if
      else {
        parent -> right = NULL;
